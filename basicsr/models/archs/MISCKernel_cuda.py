...
kernel_Kernel_updateGradVertical = '''
	extern "C" __global__ void kernel_Kernel_updateGradVertical(
		...
		for (int intFilterX = 0; intFilterX < SIZE_1(horizontal); intFilterX += 1){
			...
			for (int intChannel = 0; intChannel < SIZE_1(gradLoss); intChannel++) {
			    float floatSampled = VALUE_4(input, intSample, intChannel, top, left) * (1 + (left - position_x)) * (1 + (top - position_y)) +
			    ...
			    floatOutput += VALUE_4(gradLoss, intSample, intChannel, intY, intX) * floatSampled *
